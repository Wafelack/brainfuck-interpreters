...
impl VM {
    ...
    fn run_opcode(&mut self, opcode: OpCode) {
        match opcode {
            OpCode::Inc => self.memory[self.memory_pointer] = self.memory[self.memory_pointer].checked_add(1).unwrap_or(0),
            OpCode::Dec => self.memory[self.memory_pointer] = self.memory[self.memory_pointer].checked_sub(1).unwrap_or(255),
            OpCode::Next => if self.memory_pointer + 1 >= self.memory.len() {
                self.memory_pointer = 0;
            } else {
                self.memory_pointer += 1;
            }
            OpCode::Prev => if self.memory_pointer as isize - 1 < 0 {
            ...
            OpCode::Jmp => if self.memory[self.memory_pointer] != 0 {
